...
public class Student {
	...
	public boolean isValidMonth(String monthStr) {
		// if str length == 2, check elements on positions 0 & 1
		// if str length == 1, check elements on position 0
		// if length not in range [1;2] - return false
		if (monthStr.length() != 0)
		{
			if ((monthStr.length() == 2 && 
				...
				!Character.isAlphabetic(monthStr.charAt(0))))
			{	
				if (monthStr.length() <= 2 && monthStr.length() > 0) {
